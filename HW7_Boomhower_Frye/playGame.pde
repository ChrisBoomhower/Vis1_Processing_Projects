/******************************************************************************************
 playGame class for Round custom ball movement
 ******************************************************************************************/
class playGame {
  ...
  void playRound(int roundNum) {
    ...
    for (int i = 0; i<ballCount; i++) {
      ...
      if (ball[i].getSpeedX()         <  0
        ...
        if (ball[i].getPowerupFlag() == true) {
          paddle.setPaddleSize(paddle.getPaddleHeight()*2, paddle.getPaddleWidth());
          fcPowerup+=300;
        } else if (fcPowerup == 0) paddle.setPaddleSize(origPaddleHeight, origPaddleWidth);
